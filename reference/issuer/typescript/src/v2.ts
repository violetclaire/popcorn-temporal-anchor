...
export const TAIN_PATTERN = /^tain_[0-9A-HJKMNP-TV-Z]{26}$/;
const TAIN_PREFIX = "tain_";
...
function encodeBase32(value: bigint, length: number): string {
  ...
  for (let i = 0; i < length; i++) {
    out = CROCKFORD32[Number(remaining & 31n)] + out;
    remaining >>= 5n;
  }
  if (remaining !== 0n) {
    throw new Error("TAIN component exceeds its field width");
  }
  return out;
}

export function generateTain(witnessedAtMs: number): string {
  if (!Number.isSafeInteger(witnessedAtMs) || witnessedAtMs < 0) {
    throw new Error("witnessed_at_ms must be a non-negative safe integer");
  }
  const randomness = new Uint8Array(10);
  crypto.getRandomValues(randomness);
  let rand = 0n;
  for (const byte of randomness) rand = (rand << 8n) | BigInt(byte);
  return `${TAIN_PREFIX}${encodeBase32(BigInt(witnessedAtMs), 10)}${encodeBase32(rand, 16)}`;
}

export function isValidTain(value: unknown): value is string {
  return typeof value === "string" && TAIN_PATTERN.test(value);
}
...
function requireDigest(value: unknown, label: string): asserts value is Sha256Digest {
  ...
  requireExactKeys(value, ["algorithm", "value"]);
}
...
export async function issuePopcornWitnessReceipt(
  ...
  if (nodeId !== "767-2676.com") {
    throw new Error("node_id must be 767-2676.com for POPCORN-WITNESS/2.0");
  }
  if (!options.receipt_id || !options.signing_key_id || !options.payment_identifier) {
    throw new Error("issuer and receipt identifiers are required");
  }
  if (!/^pwr_[0-9a-f]{32}$/.test(options.receipt_id)) {
    throw new Error("receipt_id must be pwr_ plus 32 lowercase hex characters");
  }
  requireDigest(options.license_terms_digest, "license_terms_digest");
  ...
  if (options.clock_accuracy_radius_ms > 60_000) {
    throw new Error("clock_accuracy_radius_ms exceeds 60000");
  }
  ...
  const tain = generateTain(witnessedAtMs);
  ...
  const witnessReceipt = {
    ...
    issuing_origin: "https://767-2676.com",
    issuance_endpoint: "/v1/receipt",
    tain_verification_uri: `https://767-2676.com/v1/receipt/tain/${tain}`,
    contribution_license_uri: "https://767-2676.com/license/contribution/1.0",
    license_terms_digest: options.license_terms_digest,
  };
  ...
  try {
    ...
    return Response.json(response, {
      ...
      headers: {
        ...
        "X-POPCORN-Protocol": "POPCORN-WITNESS/2.0",
      },
    });
  } catch (error) {
    return Response.json(
