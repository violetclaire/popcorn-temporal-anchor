...
def verify_popcorn_witness_evidence(
    ...
    *,
    ...
    presentation_keys = {"tain_qr_png_base64", "tain_qr_payload", "letterhead_uri"}
    response_keys = {"witness_receipt", "witness_attestation", "payment_status"}
    if presentation_keys.intersection(response):
        response_keys |= presentation_keys
    ...
    _require_exact_keys(
        receipt,
        {
            ...
            "issuing_origin",
            "issuance_endpoint",
            "tain_verification_uri",
            "contribution_license_uri",
    ...
    if receipt.get("node_id") != expected_node_id:
        raise ValueError("unexpected node_id")
    if receipt.get("protocol_id") != "POPCORN-WITNESS/2.0":
        raise ValueError("unexpected protocol_id")
    if (
        not isinstance(receipt.get("receipt_id"), str)
        or not re.fullmatch(r"pwr_[0-9a-f]{32}", receipt["receipt_id"])
    ...
    if not isinstance(receipt.get("tain"), str) or not re.fullmatch(
        r"tain_[0-9A-HJKMNP-TV-Z]{26}", receipt["tain"]
    ):
        raise ValueError("witness receipt tain is missing or malformed")
    if (
        receipt.get("issuing_origin") != "https://767-2676.com"
        or receipt.get("issuance_endpoint") != "/v1/receipt"
        or receipt.get("tain_verification_uri") != "https://767-2676.com/v1/receipt/tain/" + receipt["tain"]
        or receipt.get("contribution_license_uri") != "https://767-2676.com/license/contribution/1.0"
    ):
        raise ValueError("witness receipt provenance or license URI is invalid")
    _require_sha256_digest(receipt.get("license_terms_digest"), "license_terms_digest")
    if presentation_keys.issubset(response):
        if (
            not isinstance(response["tain_qr_png_base64"], str)
            or not response["tain_qr_png_base64"]
            or response["letterhead_uri"] != receipt["tain_verification_uri"]
            or not isinstance(response["tain_qr_payload"], str)
        ):
            raise ValueError("TAIN presentation fields are invalid")
        try:
            qr = json.loads(response["tain_qr_payload"])
        except json.JSONDecodeError as exc:
            raise ValueError("TAIN QR payload is invalid JSON") from exc
        if not isinstance(qr, dict):
            raise ValueError("TAIN QR payload is invalid")
        _require_exact_keys(qr, {"v", "tain", "license_uri", "terms_digest", "verify_uri"}, "TAIN QR payload")
        if (
            qr["v"] != 1
            or qr["tain"] != receipt["tain"]
            or qr["license_uri"] != receipt["contribution_license_uri"]
            or qr["terms_digest"] != receipt["license_terms_digest"]["value"]
            or qr["verify_uri"] != receipt["tain_verification_uri"]
        ):
            raise ValueError("TAIN QR payload does not match the signed receipt")
        expected_qr = json.dumps(
            {
                "v": 1,
                "tain": receipt["tain"],
        ...
        if response["tain_qr_payload"] != expected_qr:
            raise ValueError("TAIN QR payload is not minified in canonical key order")

    request_received_ms = _utc_ms(receipt["request_received_at_utc"], "request_received_at_utc")
    witnessed_ms = _utc_ms(receipt["witnessed_at_utc"], "witnessed_at_utc")
    alphabet = "0123456789ABCDEFGHJKMNPQRSTVWXYZ"
    tain_timestamp_ms = 0
    for character in receipt["tain"][5:15]:
        tain_timestamp_ms = (tain_timestamp_ms << 5) | alphabet.index(character)
    if tain_timestamp_ms != witnessed_ms:
        raise ValueError("witness receipt tain does not match witnessed_at_utc")
